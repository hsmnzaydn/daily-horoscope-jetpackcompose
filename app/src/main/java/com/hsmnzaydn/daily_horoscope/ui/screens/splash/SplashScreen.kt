...
import com.hsmnzaydn.daily_horoscope.fake.NavControllerAndSplashViewModelProvider
...
fun SplashScreenPreview(
    @PreviewParameter(
        NavControllerAndSplashViewModelProvider::class) fakeParameter: Pair<NavController,SplashViewModel>
) {
    Column(
        modifier = Modifier
            .fillMaxHeight(1f)
        ...
        GlobalScope.launch(Dispatchers.Main) {
            delay(3000L)
            fakeParameter.first.popBackStack()
            fakeParameter.first.navigate(Navigation.HOROSCOPE_LIST_SCREEN.name)
